...
My goal: a SOC Analyst role where I contribute to threat detection, alert triage, and incident response. The best defenders understand how systems can be broken — so I build labs that reflect real attack scenarios and document every finding.
...
| 03 | Network Forensics & PCAP Analysis | What does the attack look like on packet level? Can we reconstruct the timeline? | ✅ |
| 04 | Exploitation & Detection | If Metasploit gets a shell — does the IDS see it? | 🔜 |
| 05 | Lateral Movement | Can SIEM detect movement between hosts post-compromise? | 🔜 |
...
*Self-built SOC Analyst Simulation — the full detection engineering lifecycle in my own lab.*

Going beyond pre-packaged SOC courses: stand up Splunk, ELK, and Security Onion in parallel, generate telemetry with MITRE ATT&CK techniques, write my own Sigma/SPL detections, and run timed L1 triage shifts with realistic mixed traffic. Built as a capstone on top of Aegis.
...
| 03 | Security Onion Evaluation | Integrated NSM distro with Zeek logs | 🔜 |
| 04 | Telemetry with Atomic Red Team | MITRE ATT&CK adversary emulation | 🔜 |
| 05 | Detection Engineering — Sigma, SPL, Regex | Write once, deploy everywhere | 🔜 |
...
*Started from zero. 3 exams in 5 months. Still learning, still building.* 🚀
